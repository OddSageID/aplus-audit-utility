...
class AIAnalysisResponseSchema(BaseModel):
    ...
    critical_issues: List[str] = Field(
        default_factory=list, max_length=50, description="List of critical issues"
    )

    recommendations: List[str] = Field(
        ..., min_length=1, max_length=50, description="List of recommendations"
    )
...
class FindingSchema(BaseModel):
    ...
    severity: SeverityEnum = Field(..., description="Finding severity level")
    ...
    def validate_check_id_format(cls, v):
        ...
        if not re.match(r"^[A-Z0-9\-\.]+$", v):
            raise ValueError(
                "Check ID must contain only uppercase letters, numbers, hyphens, and dots"
            )
        return v
...
class AuditConfigSchema(BaseModel):
    ...
    log_level: Annotated[
        str, StringConstraints(pattern=r"^(DEBUG|INFO|WARNING|ERROR|CRITICAL)$")
    ] = "INFO"
...
class DatabaseConfigSchema(BaseModel):
    ...
    database_url: Annotated[
        ...
        Field(
            default="sqlite:///./audit_history.db",
...
class RateLimitConfigSchema(BaseModel):
    ...
    def validate_thresholds(cls, values):
        ...
        if per_minute and per_hour:
            # Allow small configs to be equal, but prevent aggressive per-minute
            # limits from exceeding a reasonable hourly ceiling.
            if per_minute >= 10 and per_hour <= per_minute * 10:
                raise ValueError("Hourly limit should meaningfully exceed the per-minute limit")
