...
class AuditMetrics:
    ...
    def to_prometheus(self) -> str:
        ...
        add_metric("audit_risk_score", self.risk_score, "gauge", "Overall risk score (0-100)")
    ...
    def get_alerts(self) -> List[Dict[str, str]]:
        ...
        if self.collectors_failed > 0:
            failure_rate = (
                self.collectors_failed / self.collectors_total if self.collectors_total > 0 else 0
            )
...
class MetricsCollector:
    ...
    def record(
        ...
        with self._lock:
            ...
            if metric_type == MetricType.COUNTER:
                ...
                self.metric_store[name] = {"type": metric_type, "value": value}
    ...
    def get_trends(self, hostname: Optional[str] = None, limit: int = 10) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_critical_findings": round(
                sum(m.findings_critical for m in metrics) / len(metrics), 2
            ),
            "trend": trend,
            "time_range": {
    ...
    def export_all(self, format: str = "json") -> str:
        ...
        """
        if format == "json":
            return json.dumps([m.to_dict() for m in self.metrics_history], indent=2, default=str)
