...
class AuditOrchestrator:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, config: AuditConfig):
        self.config = config
        self.logger = AuditLogger.get_logger(level=config.log_level, log_file=config.log_file)
    ...
    def _finalize_metrics(self):
        ...
        self.execution_metrics.circuit_breaker_opens = rate_limiter_stats.get(
    ...
    async def _generate_remediation_scripts(self):
        ...
        priority_findings = [
            f for f in self.audit_results["all_findings"] if f["severity"] in ["CRITICAL", "HIGH"]
        ]
