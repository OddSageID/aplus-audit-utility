...
class AuditRepository:
    ...
    def _build_integrity_payload(
        self, audit_run: AuditRun, raw_results: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            audit_run = AuditRun(
                ...
                collectors_failed=sum(
                    1
                    for r in audit_results.get("collector_results", {}).values()
            ...
            loaded = (
                ...
                .options(selectinload(AuditRun.findings), selectinload(AuditRun.remediations))
                .filter(AuditRun.audit_id == audit_run.audit_id)
                .first()
            )
            return loaded
    ...
    def get_audit_run(self, audit_id: str) -> Optional[AuditRun]:
        ...
        try:
            return (
                session.query(AuditRun)
                .options(selectinload(AuditRun.findings), selectinload(AuditRun.remediations))
                .filter(AuditRun.audit_id == audit_id)
                .first()
            )
        finally:
            session.close()
    ...
    def get_audit_trends(self, hostname: Optional[str] = None, days: int = 30) -> Dict[str, Any]:
        """
    ...
    def mark_finding_resolved(
        self, finding_id: int, resolution_notes: Optional[str] = None
    ) -> bool:
    ...
    def get_remediation_by_id(self, remediation_id: int) -> Optional[RemediationExecution]:
        ...
        try:
            return (
                session.query(RemediationExecution)
                .filter(RemediationExecution.id == remediation_id)
                .first()
            )
        finally:
            session.close()
    ...
    def record_remediation_execution(
        ...
        try:
            ...
            if remediation:
                ...
                session.commit()
                self.logger.info(
                    f"Recorded execution for remediation {remediation_id}: {'success' if success else 'failed'}"
                )
                return True
            return False
        except SQLAlchemyError as e:
